...
import { buildFolderRowHtml, buildImageCardHtml } from './gallery.templates.js';
...
function renderFolders() {
  ...
  const rows = [
    ...
    ...folders
      ...
      .map((folder) => buildFolderRowHtml(folder, selectedFolder, folderDisplayName))
  ];
...
function fillPreview(image) {
  previewImage.src = `${API_BASE_URL}${image.thumbMediumUrl || image.thumbUrl || image.url}`;
...
function renderImages() {
  ...
  galleryGrid.innerHTML = pageList
    .map((image) => buildImageCardHtml(image, API_BASE_URL, selectedImages.has(image.id)))
    .join('');
